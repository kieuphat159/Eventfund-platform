...
export const MyEvents: React.FC = () => {
  ...
  const myEvents = useMemo(() => {
    ...
    return events.filter((event) => (event.organizer || '').toLowerCase() === wallet);
  }, [events, user]);
      ...
      <div className="space-y-4">
        {myEvents.length > 0 ? (
          myEvents.map((event) => {
            ...
            return (
              <Card key={event._id || event.id} className="bg-slate-900 border-slate-800 hover:border-purple-500/50 transition-all">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-semibold text-white">{event.title || 'Untitled event'}</h3>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 pt-4 border-t border-slate-800">
                      {event.ticketTiers!.map((tier, index) => (
                        <div key={index} className="bg-slate-800/50 rounded-lg p-3">
                          ...
                          <p className="text-xs text-slate-500 mt-1">{tier.totalSupply || 0} available</p>
                        </div>
                      ))}
                    </div>
                  )}
