...
# Usage: .\scripts\upload-backend-env-to-ssm.ps1 [EnvFile] [Environment]
#
# Examples:
#   .\scripts\upload-backend-env-to-ssm.ps1                             # Uses backend/.env, env=dev
#   .\scripts\upload-backend-env-to-ssm.ps1 backend\.env.prod prod      # Uses custom file, env=prod
...
if (-not (Test-Path $EnvFile)) {
    ...
    Write-Host "  Create it by copying: copy backend\.env.example backend\.env"
    exit 1
}
...
function Upload-Param {
    ...
    # Always use --overwrite, let AWS handle create vs update
...
# ─── Main ────────────────────────────────────────────────────────────────────
Log-Info "Uploading backend env to AWS Parameter Store"
